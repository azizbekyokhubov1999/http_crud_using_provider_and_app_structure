...
class NetworkService{
  ...
  //baseURL =https://65ace4a7adbd5aa31bdfb633.mockapi.io/product
  static const baseUrl = "65ace4a7adbd5aa31bdfb633.mockapi.io";

  // api = /product
static const apiOfProducts = "/product";
...
// get method
 ...
 static  Future<List<Product>> readPosts() async {
    final url = Uri.https(baseUrl, apiOfProducts);
    final response = await get(url, headers: headers);
    if (response.statusCode == 200) {
      final List<dynamic> responseData = jsonDecode(response.body);
      List<Product> products = responseData.map((data) => Product.fromJson(data)).toList();
      return products;
    } else {
